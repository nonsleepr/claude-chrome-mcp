...
import { title, section, statusLine, secure, insecureWithBg, warningBlock, code, highlightJSON, dim, path as pathColor, url, success, } from './utils/format.js';
/**
 ...
 */
function createWrapperScript(targetDir, scriptPath, runtimeInfo, options) {
 ...
 */
function hasAuthToken(wrapperDir) {
    try {
 ...
 */
function readAuthToken(wrapperDir) {
    try {
        const wrapperPath = process.platform === 'win32'
            ? path.join(wrapperDir, 'claude-chrome-mcp-native-host.bat')
            : path.join(wrapperDir, 'claude-chrome-mcp-native-host');
 ...
 */
function getConfiguredPort(wrapperDir) {
    try {
        const wrapperPath = process.platform === 'win32'
            ? path.join(wrapperDir, 'claude-chrome-mcp-native-host.bat')
            : path.join(wrapperDir, 'claude-chrome-mcp-native-host');
 ...
 */
function getConfiguredRuntime(wrapperDir) {
    try {
        const wrapperPath = process.platform === 'win32'
            ? path.join(wrapperDir, 'claude-chrome-mcp-native-host.bat')
            : path.join(wrapperDir, 'claude-chrome-mcp-native-host');
